...
const TelaDeLanches = () => {
  ...
  useEffect(() => {
    const fetchEmpresasLanches = async () => {
      try {
        const response = await fetch("http://192.168.0.101:3000/mar", {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          throw new Error(
            `Erro ao obter Frutos: ${response.statusText}`
          );
        }
        ...
        if (data.success) {
          ...
          console.error("Erro ao obter Frutos:", data.message);
        }
      } catch (error) {
  ...
  return (
    <View>
      ...
      >
        Estabelecimentos de Frutos do Mar
      </Text>
      <FlatList
