...
const TelaDeLanches = () => {
  ...
  useEffect(() => {
    const fetchEmpresasLanches = async () => {
      try {
        const response = await fetch(`${ROTA_URL}/massas`, {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          throw new Error(
            `Erro ao obter Massas: ${response.statusText}`
          );
        }
        ...
        if (data.success) {
          ...
          console.error("Erro ao obter Massas:", data.message);
        }
      } catch (error) {
