...
const TelaDeLanches = () => {
  ...
  useEffect(() => {
    const fetchEmpresasLanches = async () => {
      try {
        const response = await fetch(`${ROTA_URL}/salgados`, {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          throw new Error(
            `Erro ao obter Salgados: ${response.statusText}`
          );
        }
        ...
        if (data.success) {
          ...
          console.error("Erro ao obter Salgados:", data.message);
        }
      } catch (error) {
